...
APPS = [
    ...
    },
    {
        ...
        "changelog": [
            ...
            {
                "date": "2026-09-22",
                "note": "Windows用接続ガイド(AquaLink-windows-setup.zip)を修正。AquaLinkの既定ポート(8091)がWindows標準のWebClient設定では許可対象外で、認証情報が送られず「認証されていません」(エラー1244)とだけ表示され原因が分かりにくかった問題を修正。WebClientの許可リスト(AuthForwardServerList)は「http://サーバー名」形式(ポート番号なし)で登録する必要があり、単純な「*」は正式な書式ではなく実際には効いていなかったため、正しい形式に修正した上で、既存の登録内容を上書きせず追記する方式に変更(複数のAquaLink共有を別名で使う場合に備えて)。あわせて、この修正後に見つかったもう1件も対処: サーバー名だけを指定する通常のUNCパスはWebClient側で常にポート80とみなされるため、AquaLink既定の8091には別途「サーバー名@ポート番号」形式での指定が必要だった(「ネットワークパスが見つかりません」エラー67の原因)。実際にWindows 11実機で確認",
    ...
    },
    {
        ...
        "platform_en": "For PowerPC Macs (Tiger or later)",
    ...
    },
    {
        ...
        "platform_en": "For PowerPC Macs (Tiger/Leopard)",
    ...
    },
    {
        ...
        "platform_en": "For Intel Macs (Mavericks–High Sierra)",
